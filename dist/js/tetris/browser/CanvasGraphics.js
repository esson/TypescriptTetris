import { TETRIS_SETTINGS_DEFAULT } from './Settings.js';
...
class CanvasGraphics {
    constructor(tetris, settings = TETRIS_SETTINGS_DEFAULT, scale = 20, canvas = null) {
        this.tetris = tetris;
    ...
    drawTextFlash(animationTimeElapsed, text) {
    ...
    drawWell() {
        let wellPosition = this.wellOffset, wellSize = new Vector(this.tetris.columns * this.scale, this.tetris.lines * this.scale), wellLineWidth = 2;
        // Draw the background.
        //
            ...
            for (let y = 0; y < this.tetris.well.matrix.length; y++) {
                for (let x = 0; x < this.tetris.well.matrix[y].length; x++) {
                    if (this.tetris.well.matrix[y][x] !== 0) {
                        let position = new Vector(x, y).multiply(this.scale).add(this.wellOffset), color = this.settings.colors.tetrimino[this.tetris.well.matrix[y][x]];
    ...
    drawTextOverlay() {
        ...
        if (this.tetris.state !== TetrisState.Gameplay) {
            ...
            if (this.tetris.state === TetrisState.Start) {
                ...
                // Draw the Title
                //
                const titleText = 'TETRIS', titlePosition = wellCenterPosition.subtract(new Vector(0, this.scale * Math.floor(textLines.length / 2)));
    ...
    drawBlock(position, fill, stroke) {
    ...
    drawPreview() {
        const previewCount = 1, previewSize = new Vector(this.asideSize.x, this.asideSize.x * previewCount), previewPosition = new Vector(this.asideOffset.x, this.asideOffset.y);
        ...
        if (this.tetris.state !== TetrisState.Paused && this.tetris.state !== TetrisState.Start) {
            for (let i = 0; i < previewCount; i++) {
                ...
                let tetriminoSize = new Vector(tetrimino.getWidth() * this.scale, tetrimino.getHeight() * this.scale), tetriminoPosition = previewPosition
    ...
    drawScore() {
        let size = new Vector(this.asideSize.x, 1 * this.scale), position = new Vector(this.asideOffset.x, this.asideOffset.y + 8 * this.scale).add(new Vector(size.x / 2, 0));
    ...
    drawTetrimino(tetrimino = this.tetris.player.tetrimino, position = this.tetris.player.position.multiply(this.scale).add(this.wellOffset), fill, stroke) {
        if (this.tetris.state === TetrisState.Paused || this.tetris.state === TetrisState.Start) {
            return;
        }
    ...
    getLinearGradientFillStyle(color, fillHeight) {
        let colors = color.map(x => new Color(x));
